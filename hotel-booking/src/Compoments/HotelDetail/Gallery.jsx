            ...
            {isModalOpen && (
                <div className="">
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                        <div className="w-[90%] relative md:w-2/3 h-[85%] bg-white p-6 rounded-lg mt-0 md:mt-12 overflow-y-scroll">
                            <div className="flex justify-between items-center pb-4">
                                <div className="flex justify-start items-center gap-3">
                                    ...
                                    <p className="font-semibold text-md md:text-lg">The Venetian Resort Las Vegas</p>
                                </div>
                                <button
                                    className="px-6 py-2 text-xs md:text-lg bg-blue-600 text-white rounded-md hidden md:block"
                                >
                                    Reserve a room
                                </button>
                            </div>
                            {/* Tabs */}
                            <div className="flex space-x-4 mb-4 border-b pb-2 overflow-x-scroll scrollbar-hide">
                                {tabCategories.map((tab) => (
                                    <button
                                        key={tab.key}
                                        onClick={() => setActiveTab(tab.key)}
                                        className={`px-4 font-medium py-2 rounded text-xs md:text-sm whitespace-nowrap flex-shrink-0 ${activeTab === tab.key
                                                ? "bg-gray-800 text-white"
                                                : "bg-gray-200"
                                            }`}
                                    >
                                        {tab.name} ({galleryTabData[tab.key].length})
                                    </button>
                                ))}
                            </div>
                            ...
                            {/* Close Button */}
                            <div className="fixed bottom-16 right-6 bg-white px-12 py-3">
                                <button className="text-sm font-medium md:text-lg z-50 px-3 py-2 bg-blue-600 text-white rounded-md block md:hidden">
                                    Reserve a room
                                </button>
                            </div>
                        </div>
